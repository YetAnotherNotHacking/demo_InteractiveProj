...
class InteractiveProjectorTracker:
    ...
    def calibrate(self):
        ...
        print("Point to each calibration point as they appear and press SPACE key")
    ...
    def detect_finger(self, frame):
        ...
        results = self.hands.process(frame_rgb) # Main tracker with hands, assuming they are visible (fall back to the Pose detection of the wrist of it can't find hands)
        ...
        if pose_results.pose_landmarks:
            wrist = pose_results.pose_landmarks.landmark[15]  # hard coded for the left wrist, will rework
            x = int(wrist.x * frame_width)
            y = int(wrist.y * frame_height)
            return (x, y)       
        return None  
    def check_in_bounds(self, point):
        if point[0] < 0 or point[0] >= self.projector_width:
            return False
        if point[1] < 0 or point[1] >= self.projector_height:
            return False
        return True 
    def map_to_projector(self, finger_pos):
        ...
        if self.check_in_bounds(transformed_point[0][0]):
            print(f'Debug Interface: {transformed_point[0][0]}')
        elif self.check_in_bounds(transformed_point[0][0]) == False:
            print(f'Debug Interface: pointerException: out of bounds')
